    ...
    var isExpiredToken: Bool {
        ...
        if self.code == 401 {
               ...
               let authenticateHeader = reponseHeaders["WWW-Authenticate"] ?? reponseHeaders["Www-Authenticate"]
            {
                ...
                if let error = headerDictionary["error"],
                   error == "invalid_token" || error == "expired_token" || error == "\"invalid_token\""
                {
                    return true
                }
            }
                ...
                if let errors = jsonDic["errors"] as? [[String: AnyObject]] {
                    for error in errors {
                        if let errorType = error["errorType"] as? String, errorType == "invalid_token" || errorType == "expired_token" {
                            return true
                        } else if let urlString = self.userInfo[NSURLErrorFailingURLErrorKey] as? String,
                                  urlString.contains("api.twitter.com"), let errorCode = error["code"] as? Int, errorCode == 89
                        {
