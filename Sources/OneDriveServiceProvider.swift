...
public class OneDriveServiceProvider: CloudServiceProvider {
    ...
    public enum Route {
        ...
        var drives: String {
            ...
            case let .drive(driveId): return "drives/\(driveId)"
            case let .group(groupId): return "groups/\(groupId)/drives"
            case let .site(siteId): return "sites/\(siteId)/drives"
            case let .user(userId): return "users/\(userId)/drives"
            }
        }
        ...
        func url(of subpath: String) -> URL {
            ...
            case let .drive(driveId): path = "drives/\(driveId)/" + subpath
            case let .group(groupId): path = "groups/\(groupId)/drive/" + subpath
            case let .site(siteId): path = "sites/\(siteId)/drive/" + subpath
            case let .user(userId): path = "users/\(userId)/drive" + subpath
            }
    ...
    /// Delete a DriveItem by using its ID or path. Note that deleting items using this method will move the items to the recycle bin
    ...
    /// Search the hierarchy of items for items matching a query. You can search within a folder hierarchy, a whole drive, or files shared
