...
public protocol CloudServiceProvider: AnyObject, CloudServiceResponseProcessing {
    ...
    /// Note: The access token of some cloud service (eg: OneDrive) are short time. So we need a refresh token to refresh the access token
    /// when expired.
...
struct ISO3601DateFormatter {
    ...
    func date(from dateString: String) -> Date? {
        secondsDateFormatter.date(from: dateString)
