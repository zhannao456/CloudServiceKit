...
public final class HTTPResult: NSObject {
    ...
    public var statusCode: Int? {
        (self.response as? HTTPURLResponse)?.statusCode
    }

    public var text: String? {
        content.flatMap { String(data: $0, encoding: encoding) }
    }
    ...
    public lazy var cookies: [String: HTTPCookie] = {
        ...
        if let headers = self.response?.HTTPHeaders, let url = self.response?.url {
            foundCookies = HTTPCookie.cookies(
    ...
    public lazy var links: [String: [String: String]] = {
        ...
        for s in content.components(separatedBy: ", ") {
            ...
            if linkComponents.count > 1 {
                ...
                for s in linkComponents.dropFirst() {
...
public final class HTTP: NSObject, URLSessionDelegate, JustAdaptor {
        ...
        -> Data?
    {
        ...
        if !body.isEmpty {
            body.append(
                "--\(self.defaults.multipartBoundary)--\r\n"
                    .data(using: defaults.encoding)!
            )
        }
    ...
    public func request(
        ...
        var requestResult: HTTPResult = HTTPResult(
            data: nil,
            response: nil,
            error: syncResultAccessError,
            task: nil
        )
