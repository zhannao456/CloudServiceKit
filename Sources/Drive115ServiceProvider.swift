...
public class Drive115ServiceProvider: CloudServiceProvider {
        ...
        get(url: url, params: params) { response in
            ...
            case let .success(result):
                   ...
                   let list = object["data"] as? [[String: Any]]
                {
                    ...
                    if let totalCount = object["count"] as? Int, let offset = object["offset"] as? Int,
                       let limit = object["limit"] as? String, let limitInt = Int(limit)
                    {
                        if offset + limitInt < totalCount {
                            completion(.success((String(offset + limitInt), items)))
                        } else {
        ...
        try await withCheckedThrowingContinuation { continuation in
            getDownloadUrl(of: item) { result in
                ...
                case let .success(url):
                    ...
                    request.setValue("BoxPlayer", forHTTPHeaderField: "User-Agent")
    ...
    public func getDownloadUrl(of item: CloudItem, parameters: [String: Any] = [:], completion: @escaping (Result<URL, Error>) -> Void) {
        ...
        post(url: url, data: data, headers: ["User-Agent": "BoxPlayer"]) { response in
            switch response.result {
            case let .success(result):
